...
# Blazegrpah command: java -server -Xmx1g -jar blazegraph.jar
...
class Category(IdentifiableEntity):
    ...
    # Marti: I added these two functions here (__eq__ and most importantly __hash__) to see if I can fix the issue of `TypeError: unhashable type: 'Category'` in the FullQuery
    # to be able to use sets for implemented customised object (by default not hashable)
...
class JournalUploadHandler(UploadHandler):
    def createJournalGraph(self, csv_file: str) -> rdflib.Graph: # Martina
        ...
        for idx, row in journals.iterrows():
            ...
            combined_ids = str(combined_ids)        # Marti: for the TypeError of 'float' (last method of FQE)
...
class BasicQueryEngine:
    ...
    def getEntityById(self, id: str) -> Optional[IdentifiableEntity]: # * Nico, working with journals, areas, and categories
        ...
        if not isinstance(id, str):     # Marti: another good measure to avoid 'float' Type Errors (hopefully)
            try:
                id = str(id)
            except Exception as e:
                print(f"Warning: Could not convert id {id} (type {type(id)}) to string. Error: {e}")
                return None
...
class FullQueryEngine(BasicQueryEngine): # all the methods return a list of Journal objects
    ...
    def getDiamondJournalsInAreasAndCategoriesWithQuartile(self, areas_ids: set[str], category_ids: set[str], quartiles: set[str]) -> list[Journal]: # ?? Ila, Marti & Nico requires testing once other methods work
        ...
        target_areas = {self.getEntityById(a_id) for a_id in areas_ids} if areas_ids else set() # self.getAllAreas() if not areas_ids else [self.getEntityById(area) for area in areas_ids] # Ila fixed this part that she wrote wrong
        target_categories = {self.getEntityById(cat_id) for cat_id in category_ids} if category_ids else set() # self.getAllCategories() if not category_ids else [self.getEntityById(category) for category in category_ids]
        
        # Marti, trying it like this (keeping modifications from everyone as well):
        for journal in all_journals:
            journal.id = str(journal.id)        # Marti: added to see if I can force-cast the id to a string type, to avoid the float error
            if journal.hasAPC() == False:
                continue

            area_match = False
            journals_areas = journal.getAreas()
            if not areas_ids:
                if journals_areas:
                    area_match = True
            else: # if areas_ids are specified
                if any(area in target_areas for area in journal.getAreas()):
                    area_match = True
            
            if not area_match: # if the matching happened either from all of from the target,  let's go on
                continue 

            category_match = False
            journals_categories = journal.getCategories()
            if not category_ids:
                if not quartiles:
                    if journals_categories:
                        category_match = True
                else:
                    if any(cat.getQuartile() in quartiles for cat in journals_categories):
                        category_match = True
            else:
                for category in journals_categories:
                    if category in target_categories:
                        if not quartiles or category.getQuartile() in quartiles:
                            category_match = True
                            break

            if area_match and category_match:
